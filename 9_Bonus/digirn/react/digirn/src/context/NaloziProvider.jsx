...
const NaloziProvider = ({children}) =>{
  ...
  const dodajNalog = (noviNalog) => {
    ...
    setNalozi(trenutniNalozi =>{
      ...
      return {listaNaloga: azuriraniNalozi, dodajNalog, obrisiNalog, izmjeniNalog, ucitajNaloge, spremiNaloge};
    }) 
  ...
  const obrisiNalog = (nalogId) => {
    ...
    setNalozi(trenutniNalozi =>{
      const azuriraniNalozi = trenutniNalozi.listaNaloga.filter(nalog => nalog.id !== nalogId);
      return {listaNaloga: azuriraniNalozi, dodajNalog, obrisiNalog, izmjeniNalog, ucitajNaloge, spremiNaloge};
    }) 
  }
  ...
  const izmjeniNalog = (noviNalog) => {
    ...
    setNalozi(trenutniNalozi =>{
      const azuriraniNalogIndex = trenutniNalozi.listaNaloga.findIndex(nalog => nalog.id === noviNalog.id);
      const azuriraniNalozi = trenutniNalozi.listaNaloga.map((nalog,index) => index === azuriraniNalogIndex ? noviNalog : nalog);
  ...
  const spremiNaloge = async () => {
    ...
    try {
      ...
      await fetch('https://algebra-fe-default-rtdb.europe-west1.firebasedatabase.app/digirn.json', {
        method: 'DELETE',
      });
  ...
  return (
    <NaloziContext value={{nalozi, dodajNalog, obrisiNalog, izmjeniNalog, ucitajNaloge, spremiNaloge}}>{children}</NaloziContext>
  );
